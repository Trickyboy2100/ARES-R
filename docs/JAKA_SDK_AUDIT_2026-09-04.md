...
`config/robot_world.json` 采用 `+X` 车体前方、`+Y` 车体左侧、`+Z` 向上。左臂基座为 `(0,+0.200,0.120)m / yaw +45°`，右臂基座为 `(0,-0.200,0.120)m / yaw -45°`。两基座水平间距 400 mm，安装高度 120 mm，朝向夹角 90°。实时控制器 TCP 经过固定基座变换后只用于 Terminal 显示；姿态显示暂按常规 RPY 组合，正式规划前须与 JAKA V2.1.5 姿态约定交叉验证。
