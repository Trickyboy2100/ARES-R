...
`config/robot_world.json` 采用 `+X` 北/车体前方、`+Y` 西/车体左侧、`+Z` 向上，东为 `-Y`。左臂基座为 `(0,+0.200,1.200)m / JAKA yaw +135°`，右臂基座为 `(0,-0.200,1.200)m / JAKA yaw +45°`。控制器 `q=0` 的伸展方向主要沿各自基坐标 `-Y`，经上述变换后分别指向西北和东北。J1 外壳归中朝向为镜像关系：左臂东北、右臂西北。两基座水平间距 400 mm，安装高度 1.200 m。实时 TCP 经过固定基座变换后只用于 Terminal 显示；姿态显示暂按常规 RPY 组合，正式规划前须与 JAKA V2.1.5 姿态约定交叉验证。
...
逐关节模型审计发现：JAKA 官方 ROS 2 仓库的 MiniCobo URDF 关节尺寸约为 `187、210、210.5、159.3 mm`，官方硬件手册给出的完全伸直总尺寸约为 766.8 mm。左臂工具沿末端延伸约 113.9 mm，理论底座到 TCP 总长度约 880.7 mm，与现场 `q=0` 控制器结果约 881.4 mm 一致。此前把 580 mm 工作半径当作底座到 TCP 总长度并据此判定模型冲突，属于错误解释，现已撤销。

官方 ROS URDF 的关节零位不是现场控制器侧装零位：直接输入相同的六个零角度不会得到相同姿态。公开研究给出的侧装 MiniCobo 模型采用 MDH `Rx(alpha) Tx(a) Rz(theta) Tz(d)`，并含固定关节角偏置。逐关节折线仍须完成“控制器角度 → 侧装 MDH角度 → 机身世界”的映射，并用多组只读 `kine_forward()` 验证。差异闭环前，Terminal 不显示伪造的 J1—J6 折线。
