...
`config/robot_world.json` 采用 `+X` 车体前方、`+Y` 车体左侧、`+Z` 向上。左臂基座为 `(0,+0.200,0.120)m / JAKA yaw +135°`，右臂基座为 `(0,-0.200,0.120)m / JAKA yaw -135°`。两侧外壳“正面”按 JAKA 基坐标 `-X` 解释，因而分别指向右前和左前，互呈 90°。左臂 `q=0` 的 SDK 正解主要沿基坐标 `-Y`，经 `+135°` 变换后指向机身左前，与现场姿态一致。两基座水平间距 400 mm，安装高度 120 mm。实时 TCP 经过固定基座变换后只用于 Terminal 显示；姿态显示暂按常规 RPY 组合，正式规划前须与 JAKA V2.1.5 姿态约定交叉验证。
