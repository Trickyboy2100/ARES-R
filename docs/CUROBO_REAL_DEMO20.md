...
- `reset`：当前实际关节到保存起点的独立 cuRobo 规划；已经到位则不发送 servo。复位专用范围：每关节相对起点不超过 150°、规划 TCP 行程不超过 1.5 m、时长不超过 240 s。150°来自现场手动姿态到 `ready` 的 J4 实测差值 136.3°并保留约 13.7°余量；现场关节限位、3°/s 速度、0.2 rad/s² 加速度、右侧工作区和碰撞检查继续适用，这不是任意姿态均可无条件回程。
...
7. 当前 3 倍配置的 demo 段限制：每关节相对起点不超过 20°，峰值不超过 3°/s、加速度不超过原现场 0.2 rad/s²（约 11.46°/s²）；规划 TCP 峰值不超过 60 mm/s、行程 180..220 mm。实际 TCP 相对起点位移上限：demo 段 250 mm、独立复位段 1500 mm。跟踪误差超过 0.2°或读取/发送超预算即停止。微动路径及双臂共用 site 配置未修改。
...
- 两段式更新：从现场实际偏离起点约 J1 91.6°/J5 103.6°的姿态，GPU 成功规划回保存起点；TCP 规划行程 770.522 mm、808 点、64.56 s、峰值关节速度 2.396°/s。现场数值、参考点、工具、工作区和载荷生成检查通过，未执行运动。97 项离线测试通过。
