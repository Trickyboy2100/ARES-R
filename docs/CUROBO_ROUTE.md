...
旧基线中只可复用：cuRobo 配置结构、`MotionPlanner` 初始化、独立进程隔离和关节轨迹导出。MiniCobo URDF、碰撞体、关节限位和运动学参数不得用于现场 Mini2。
...
2. 从官方 Mini2 模型起步，与实物铭牌/JAKA APP 和控制器软限位逐项核对 URDF；禁止由 MiniCobo 模型改名代替。
