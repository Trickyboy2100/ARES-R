...
机身世界坐标定义保存在 `config/robot_world.json`：`+X` 为车体前方、`+Y` 为车体左侧、`+Z` 向上；左右臂基座分别位于 `(0,+0.200,1.200)m` 和 `(0,-0.200,1.200)m`。JAKA 基坐标偏航分别为 `+135°` 和 `-135°`；两侧外壳“正面”按 JAKA 基坐标 `-X` 解释，分别指向右前和左前。`jaka-readonly` 每次刷新显示基座和实时 TCP 世界坐标；`world view` 显示经过基座、J1—J6、实时 TCP 的俯视/后视/右视折线。`-` 表示侧装 MiniCobo MDH 连杆，`:` 表示模型 J6 到 SDK 实时 TCP；重合关节在低分辨率 ASCII 图中可能覆盖。
