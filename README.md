...
机身世界坐标定义保存在 `config/robot_world.json`：`+X` 为车体前方、`+Y` 为车体左侧、`+Z` 向上；左右臂基座分别位于 `(0,+0.200,1.200)m` 和 `(0,-0.200,1.200)m`。JAKA 基坐标偏航分别为 `+135°` 和 `-135°`；两侧外壳“正面”按 JAKA 基坐标 `-X` 解释，分别指向右前和左前。左臂零关节位姿的 SDK 正解验证了连杆沿机身左前伸展。`jaka-readonly` 每次刷新显示基座和实时 TCP 世界坐标；`world view` 额外显示三视 ASCII 投影和基座到 TCP 的点状空间连线。点状连线只是空间跨度示意，不代表各关节和连杆的真实正运动学姿态。

逐关节折线当前保持锁定。官方 Mini 系列完全伸直外形尺寸约为 766.8 mm，现场左工具沿末端再延伸约 113.9 mm，合计约 880.7 mm，与控制器左臂 `q=0` TCP 距离约 881.4 mm 一致；此前把 580 mm 工作半径误当成底座到工具总长度的判断已经撤销。剩余缺口是侧装 MDH、控制器关节零位与官方 ROS URDF 零位的映射。只有模型正解与控制器 `kine_forward()` 在多组离线样本上一致后，才显示 `J1—J6—TCP`，禁止用均分点伪造关节位置。
