...
逐关节折线当前保持锁定：官方 Mini2/MiniCobo 资料标称臂展 580 mm，而现场控制器左臂 `q=0` 的 TCP 到基坐标原点距离约 881 mm。官方 MiniCobo URDF 无法复现现场控制器正解，说明型号、控制器坐标、安装设置或运动学模型仍有未核实项。只有模型正解与控制器 `kine_forward()` 在多组离线样本上一致后，才显示 `J1—J6—TCP`，禁止用均分点伪造关节位置。
