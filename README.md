...
机身世界坐标定义保存在 `config/robot_world.json`：`+X` 为车体前方、`+Y` 为车体左侧、`+Z` 向上；左右臂基座分别位于 `(0,+0.200,0.120)m` 和 `(0,-0.200,0.120)m`，偏航角分别为 `+45°` 和 `-45°`。`jaka-readonly` 每次刷新显示基座和实时 TCP 世界坐标；`world view` 额外显示俯视、后视与右视 ASCII 投影、基座到 TCP 的点状空间连线，以及控制器中当前启用的工具 TCP。点状连线只是空间跨度示意，不代表 Mini2 各关节和连杆的真实正运动学姿态。
