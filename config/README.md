...
`robot_world.json` 保存双臂基座相对机身的固定安装关系。坐标轴固定为 `+X` 前、`+Y` 左、`+Z` 上，原点是两基座中心连线中点在地面的投影。JAKA 基坐标和外壳“正面”不是同一轴；当前按外壳正面为基坐标 `-X` 记录。`jaka_mini2_kinematics.readonly.json` 保存两台控制器只读返回的原始 DH 校准值和零关节正解，单位与 DH 约定完全核实前禁止用于运动。上述配置只用于显示与规划建模，不改变控制器坐标系，也不产生运动命令。
