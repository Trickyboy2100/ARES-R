...
- 部署后用停止后的新鲜姿态做 2×恢复规划验证，cuRobo 得到 0.8708 m 候选路径，但其世界坐标最大 Y=+0.1777 m，违反右臂 Y≤-0.07 m 隔离门槛，故未生成执行载荷、未运动。当前姿态自身已在中线左侧附近，不能在未读取左臂/未建立双臂碰撞场景时自动放宽。恢复规划失败现记录 `motion_auto_recovery_blocked` 并返回明确原因。
