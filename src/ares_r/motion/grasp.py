...
def _profile(config: Dict[str, object], detection: DetectionResult) -> Dict[str, object]:
    epic = config.get("epic")
    if not isinstance(epic, dict) or not isinstance(epic.get("task_profiles"), dict):
        raise RuntimeError("config has no epic.task_profiles; global grasp/frame configuration is forbidden")
    name = detection.meta.get("epic_profile")
    profile = epic["task_profiles"].get(name)
    if not isinstance(profile, dict):
        raise RuntimeError("detection Epic profile %r is absent from config" % name)
    return profile


def _settings(profile: Dict[str, object]) -> Dict[str, object]:
    settings = profile.get("grasp")
    if not isinstance(settings, dict):
        raise RuntimeError("Epic task profile has no grasp section; refusing to invent an approach distance")
    return settings


def _check_frame(profile: Dict[str, object], detection: DetectionResult,
                 allow_unverified: bool) -> bool:
    """The pose frame must be named, stable, and verified before it is trusted."""
    declared = str(profile.get("output_frame", ""))
    if not declared:
    ...
    if reported != declared:
        raise RuntimeError("the detection was produced in frame %r, not the configured %r"
                           % (reported, declared))
    verified = profile.get("state") == "COMMISSIONED"
    if not verified and not allow_unverified:
        raise RuntimeError(
            "Epic task profile is not COMMISSIONED: complete frame/orientation/tool validation "
            "before any pose is handed to a planner")
    return verified
...
def build_grasp_plan(config: Dict[str, object], detection: DetectionResult,
    ...
    profile = _profile(config, detection)
    grasp = _settings(profile)
    arm = arm or str(profile.get("arm", ""))
    if arm not in ("left", "right"):
        raise ValueError("arm must be left or right, got %r" % (arm,))
    if profile.get("arm") != arm:
        raise RuntimeError("the Epic profile is assigned to %r, not %r"
                           % (profile.get("arm"), arm))
    frame_verified = _check_frame(profile, detection, allow_unverified_frame)
    ...
    rotation = rotation_matrix(pose.rx, pose.ry, pose.rz,
                               str(profile.get("orientation_convention", "UNKNOWN")))
