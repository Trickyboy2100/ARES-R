...
from ..contracts import (CapabilityRequirement as Cap, LockMode, LockRequirement as Lock,
...
def _skill(skill_id: str, layer: SkillLayer, description: str,
           ...
           effects: Tuple[str, ...]) -> SkillDefinition:
    return SkillDefinition(skill_id, 1, "1.0.0", description, layer,
        ...
        tuple(Lock(item.selector, LockMode.EXCLUSIVE_MOTION if motion else LockMode.EXCLUSIVE_STATE)
              for item in resources), TimeoutPolicy(10, 60, 10), COMMON_FAILURES,
        PlannerExposure.LLM, SafetyClass.S2_MOTION if motion else SafetyClass.S1_REVERSIBLE,
...
TIER_A_SKILLS = (
    _skill("observe.capture_scene", SkillLayer.L1_BASIC, "Capture a coherent scene observation.",
           ...
           ("scene.capture",), (), False, ("observation_epoch_committed",)),
    ...
    _skill("manipulation.grasp", SkillLayer.L1_BASIC, "Grasp and verify attachment of a physical resource.",
           ...
           (Res("object", ("Container", "Tool")),), True, ("object_attached",)),
    ...
    _skill("manipulation.press", SkillLayer.L2_MANIPULATION, "Press a named control affordance.",
           ...
           (Res("control", ("Fixture", "Device")),), True, ("control_state_changed",)),
    _skill("manipulation.turn", SkillLayer.L2_MANIPULATION, "Turn a named rotary control to a typed setting.",
           ...
           ("motion.plan", "motion.execute", "control.verify"),
           (Res("control", ("Fixture", "Device")),), True, ("control_setting_verified",)),
